...
- **Financier on-chain passkey verification = Ζ-GATE (hard prerequisite, NOT "future work").** In v1, `open_standby` verifies the USER's passkey on-chain (consent — the security property) but identifies the FINANCIER only by their `financier_swig` account + a wallet-layer tx signature. The financier is NOT passkey-verified on-chain. **Why it's this way: a TECHNICAL constraint, not a design judgment** — the verifier (`introspect_simd_0075`) addresses exactly one secp256r1 precompile sibling at one fixed position, so one passkey per tx; the user's is the one that had to be it (consent is load-bearing, the financier offering to lend can't harm anyone in v1). **Why it's fine in v1:** Branch controls both sides; there is no real financier whose intent must be ironclad. **Why it's REQUIRED for Ζ (the two real reasons, not symmetry-for-its-own-sake):** (1) **non-repudiation** — a real financier disputing "I never agreed to back that vault" is rebutted far more strongly by a passkey-bound op-message ("your biometric hardware signed these exact terms") than by a wallet signature; (2) **hot-key delegation** — an automated financier may co-sign with a SERVER key, not their root passkey, so their commitment could be "proven" by a key that isn't really them; real money at scale must root the commitment in the financier's actual passkey. **The fix, when built, must be ASYNCHRONOUS — NOT two-passkeys-in-one-transaction.** Do NOT build the clumsy synchronous shape (two precompile siblings in one tx forcing the financier and user to passkey-sign *simultaneously*) — that doesn't fit how real financiers operate (an institution or automated backer won't sit waiting to co-sign each user's opt-in in real time). The correct shape is **pre-sign-and-redeem**: the financier passkey-signs their backing INTENT once, ahead of time (off-line/async), producing a signed commitment; that commitment is then REDEEMED/consumed when a user opts in via `open_standby`, with the program verifying the financier's stored signature against their passkey. Same goal (strong, passkey-rooted, non-repudiable financier intent), async mechanism — it does NOT require the user and financier to be online together, and it does NOT necessarily require the two-precompile-in-one-tx verifier rewrite (the financier's signature can be verified against a stored commitment rather than a same-tx sibling). DELIBERATELY not built in v1 (YAGNI + don't speculatively build security-critical auth nothing exercises yet), but it is a HARD GATE before any real external financier onboards — not optional polish. (Flagged by the Phase-1/synthesis agent; Branch's correct itch. The async-pre-sign shape — vs. the literal synchronous two-passkey reading — was the agent's follow-up refinement: capture the RIGHT design now so a future builder doesn't build the clumsy literal one.)
...
```
(Follow the exact account-struct + secp256r1 sibling-instruction verification pattern from `register_session_key.rs` / `recover_abandoned_lock.rs`. The op-message domain string is new: `"open_standby"`. NOTE the verifier reality: `introspect_simd_0075` addresses exactly ONE precompile sibling at one fixed position → one passkey per tx. So v1 verifies the USER's passkey on-chain (the mandatory consent leg) and identifies the financier by their `financier_swig` account + wallet-layer co-sign. The financier's OWN on-chain passkey verification is a HARD Ζ-GATE — see the "Financier on-chain passkey verification = Ζ-GATE" design decision above; when built it must use the ASYNC pre-sign-and-redeem shape (financier passkey-signs backing intent ahead of time, redeemed at the user's opt-in), NOT a synchronous two-passkeys-in-one-tx — and is REQUIRED before any real external financier onboards, NOT optional. The v1 load-bearing requirement: the user's passkey MUST have signed, or the call fails.)
